	...
	handleSearch = (e) => {
		...
		this.props.form.validateFields((err, values) => {
			...
			let data = {
				...
				settle_creator: values.settle_creator ? values.merchant_name : '',	//操作人
				use_time: values.settle_time ? values.settle_time.map(item => item.unix()).join(',') : '',	//使用时间
			};
				...
				<Form
					...
					<Row gutter={20}>
						...
						<Col span={8} style={expand ? null : { display: 'none' }}>
							<FormItem label="结算时间" wrapperCol={this.formWrapperCol} className={globalStyles.formItemLabel}>
						...
						<Col span={8} style={expand ? null : { display: 'none' }}>
							<FormItem label="操作人" wrapperCol={this.formWrapperCol} className={globalStyles.formItemLabel}>
								{getFieldDecorator('settle_creator', {})(
									<TreeSelect
