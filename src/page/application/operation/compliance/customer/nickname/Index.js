...
export default class extends Component {
	...
	getData = () => {
		...
		NetOperation.nameAudit(data).then((res) => {
			this.setState({
	...
	pass(data) {
		let content = <Fragment>确定 <b>通过</b> 昵称为 <b>{data.nickname}</b> 的审核吗？</Fragment>;
		Modal.confirm({
			title: '确认提示',
			content,
			width: '450px',
			centered: true,
			onOk: () => {
				NetOperation.passName(data._id).then(res => {
	...
	passBatch(idList) {
		let content = <Fragment>已选<b>{idList.length}</b> 人，确定 通过 已选客户的昵称审核吗？</Fragment>
		Modal.confirm({
			title: '确认提示',
			content,
			width: '450px',
			...
			onOk: () => {
				...
				NetOperation.passNameBatch(json).then(res => {
					message.success('操作成功');
					this.clearSelect();
					this.getData();
				}).catch(res => {
	...
	refuse(data) {
		let content = <Fragment>确定 <b>拒绝</b> 昵称为 <b>{data.nickname}</b> 的审核吗？</Fragment>;
		Modal.confirm({
			title: '确认提示',
			content,
			width: '450px',
			centered: true,
			onOk: () => {
				NetOperation.refuseName(data._id).then(res => {
					message.success('操作成功');
					this.clearSelect();
					this.getData();
				}).catch(res => {
	...
	refuseBatch(idList) {
		let content = content = <Fragment>已选<b>{idList.length}</b> 人，确定 拒绝 已选客户的昵称审核吗？</Fragment>;
		Modal.confirm({
			title: '确认提示',
			content,
			width: '450px',
			...
			onOk: () => {
				...
				NetOperation.refuseNameBatch(json).then(res => {
					message.success('操作成功');
					this.clearSelect();
					this.getData();
				}).catch(res => {
	...
	render() {
		...
		return <Fragment >
					<div className={globalStyles.topWhiteBlock}>
						...
						<h3 className={globalStyles.pageTitle}>昵称审核</h3>
					</div>
