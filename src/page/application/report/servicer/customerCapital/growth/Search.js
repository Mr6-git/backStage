...
class Search extends Component {
	...
	componentWillMount() {
		...
		const period = `${startDate.format('YYYYMM')},${endDate.format('YYYYMM')}`;
		this.setState({
			data: {
				period,
			},
	...
	handleSearch = (e) => {
		...
		this.props.form.validateFields((err, values) => {
			if (!err) {
				const { type, createTime, isOut } = this.state;
				if (type == 0 && isOut) {
					Modal.info({
						title: '提示',
						okText: '确定',
				...
				const period = type == 2 ? createTime : values.period;
				let data = {
					time_type: values.time_type,
					period: this.dataFormat(period) || '',
				};
				this.setState({ data });
	...
	dataFormat(date) {
		...
		const dateType = type == 2 ? 'year' : 'month';
		const format = type == 2 ? 'YYYYMM' : 'YYYYMMDD';
	...
	changeType = (value) => {
		...
		switch (value) {
			...
			default: {
				createTime = [moment().startOf('day').add(-7, 'day'), moment().endOf('day')];
			} break;
		}
	...
	// 限制最近2个月
	// disabledDate = current => current && current > moment().endOf('day') || current < moment().subtract(2, 'months');
	rangeChange = dates => {
