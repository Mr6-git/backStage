...
class Search extends Component {
	...
	render() {
		const { form, agencyTree, isServicer } = this.props;
		const { getFieldDecorator } = form;
		const { createTime, year, data, type, isOpen } = this.state;
		const agencyData = agencyTree[0];
		return (
			<Form
							...
							<FormItem className={globalStyles.searchItem} label={<span></span>} wrapperCol={this.formWrapperCol}>
								{getFieldDecorator('agency_id', {
									...
									<Select placeholder="请选择机构" allowClear>
										<Option value={agencyData && agencyData.value}>本机构</Option>
										{agencyData && agencyData.children && agencyData.children.length && agencyData.children.map(item => (
											<Option value={item.value} key={item.key}>{item.alias}</Option>
										))}
									</Select>
								)}
							</FormItem>
