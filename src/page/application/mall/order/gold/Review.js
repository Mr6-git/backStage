...
class Review extends Component {
	constructor(props) {
		...
		this.state = {
			...
			currentServiceFee: 0,
			currentTaxFee: 0,
			phrase: ['金价浮动，无法审核通过，请重新提交兑换订单。', '服务费费更新，请重新提交兑换订单。', '税费更新，请重新提交兑换订单。'],
			reason: ''
		}
	}
	...
	componentWillMount() {
		...
		this.getGoldPrice(props.data.gold_type);
		this.getCocogcTax(props.data.order_number);
	}
	...
	getGoldPrice(type) {
		NetMall.getGoldPrice(type).then((res) => {
			this.setState({
				currentGoldPrice: res.data.price
			});
		}).catch((e) => {
			message.error(e.msg);
			this.setState({
	...
	passData(json) {
		NetMall.goldAuditPass(this.props.data.order_number, json).then((res) => {
	...
	refuseData(json) {
		NetMall.goldAuditRefuse(this.props.data.order_number, json).then((res) => {
			message.success('操作成功');
			this.props.onChange();
			this.props.onClose();
		}).catch((e) => {
	...
	render() {
		...
		const { currentGoldPrice, currentServiceFee, currentTaxFee } = state;
		...
		if (!state.isLoading) {
			if (currentGoldPrice != data.price) {
				isFloat = true;
				action = 2;
