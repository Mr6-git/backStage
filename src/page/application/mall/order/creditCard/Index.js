...
export default class extends Component {
	...
	getData = () => {
		...
		NetMall.getCreditOrder(data).then(res => {
	...
	getTotal = () => {
		...
		NetMall.getCreditTotal(data).then(res => {
			const items = res.data;
			this.setState({
	...
	creditDetail = (id) => {
		this.props.history.push(`${this.props.match.url}/credit_detail/${id}`);
	}
	...
	exportOrder() {
		...
		NetMall.exportCreditOrder(data).then((res) => {
			const items = res.data;
			if (items && items.id) {
		...
		return [
			{
				...
				render: data => {
					return <Fragment>
								<a href="javascript:;" onClick={() => { this.creditDetail(data) }}>{data}</a>
							</Fragment>
				}
			}, {
				title: '下单时间',
				...
				render: data => {
					...
					switch (data.sub_status) {
						...
						case 1: status = '待还款'; break;
						case 2: status = '已拒绝'; break;
						case 3: status = '还款成功'; break;
						case 4: status = '还款失败'; break;
						case 5: status = '退款成功'; break;
						case 6: status = '提交失败'; break;
					...
					switch (data.exception_status) {
						case 1:
							exception_status = <label className={classnames(globalStyles.tag)} style={{ background: '#EF6755' }}>还款失败</label>;
							break;
						case 2:
						...
						case 3:
							...
							break;
						case 4:
							exception_status = <label className={classnames(globalStyles.tag)} style={{ background: '#36BF9A' }}>还款成功</label>;
							break;
					}
				...
				render: data => {
					return <Fragment>
								...
								<a
									...
									disabled={data.sub_status != 4 || !this.props.checkAuth(1)}
								>退款</a>
								<Divider type="vertical" />
	...
	statusType = (data) => {
		if (data.exception_status == 1 || data.exception_status == 0) {
			return <a
						...
						disabled={((data.sub_status != 1 && data.sub_status != 3) || data.exception_status != 0)}
					>异常处理</a>
			...
			return <Popconfirm
						...
						okText="确定"
						cancelText="取消"
						onConfirm={() => { this.hasRemit(data.order_number) }}
					>
						<a
							href="javascript:;"
							disabled={data.exception_status == 3 && data.sub_status == 3 ? false : true}
						>已打款</a>
					</Popconfirm>
		}
	}
	...
	hasRemit = (id) => {
		...
		NetMall.confirmFinish(id, data).then(res => {
			message.success('操作成功');
			this.getData();
		}).catch(err => {
			if (err.msg) {
	...
	render() {
		...
		return <Fragment>
					<div className={globalStyles.topWhiteBlock} style={{ border: '0' }}>
						...
						<h3 className={globalStyles.pageTitle}>信用卡还款单</h3>
					</div>
					<div className={globalStyles.content}>
						<Card className={classnames(globalStyles.mBottom16)} bodyStyle={{ padding: '24px 24px 10px 24px' }} bordered={false}>
							...
							<div className={globalStyles.mBottom16}>
								...
								{filtrate.map((item, index) =>
									(<Button className={classnames(globalStyles.mRight8, btnIndex == index ? styles.checkBtn : '')} onClick={() => this.orderFilter(item.key, index)}>{item.name}</Button>)
								)}
						...
						{state.data.length ? (
							<Card className={classnames(globalStyles.mBottom24, globalStyles.statistic)} bodyStyle={{ padding: '24px' }} bordered={false}>
								<Row gutter={16} className={globalStyles.mBottom24}>
									<Col span={7}><Statistic value={totalNaked != null ? utils.formatMoney(totalNaked / 100) : '暂未统计'} title="还款金额累计" precision={2} /></Col>
						...
						/>
						<Route
							path={`${this.props.match.path}/credit_detail/:creditId`}
							children={(childProps) => {
								return <Drawer
											title="订单详情"
											placement="right"
										...
										>
											<Detail
												{...this.props}
												id={childProps.match ? childProps.match.params.creditId : null}
												getData={this.getData}
												isCompliance={true}
												allowManage={true}
											/>
