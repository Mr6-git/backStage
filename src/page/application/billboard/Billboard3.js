...
export default class extends Component {
	...
	getData() {
		...
		NetMarket.getBillboard(data).then(res => {
			if (res.code == 200 && JSON.stringify(res.code.data) != '{}') {
				if (this.state.event_id != '' && res.data.event_id != this.state.event_id) {
					//更换赛事清空柱状数据
					this.columnData = [];
				}
				this.setState({
	...
	getEventOdds() {
		...
		NetMarket.getEventTrades(data).then(res => {
			...
			if (rows && rows.length) {
				...
				this.getOddsLogs(info[2]._id, 'draw')
			} else {
				this.setState({
					display: 0
				});
	...
	// 计算实时支持率方法
	supportCount = (odds_one, odds_two, odds_thr, type) => {
		const { data } = this.state;
		if (odds_one != undefined && odds_two != undefined && odds_thr != undefined) {
			let red_probability = (1 / (1 / (odds_one.odds) + 1 / (odds_two.odds) + 1 / odds_thr.odds)) / odds_one.odds,
				blue_probability = (1 / (1 / (odds_one.odds) + 1 / (odds_two.odds) + 1 / odds_thr.odds)) / odds_two.odds;
			return red_probability / (blue_probability + red_probability);
		}
	}

	domRender = (data, flag) => {
		const { oddsDetails_one, oddsDetails_two, oddsDetails_thr } = this.state;
		let oddsInfo_one = data.length > 0 ? data[0] : null,
			oddsInfo_two = data.length > 0 ? data[1] : null,
			oddsInfo_thr = data.length > 0 ? data[2] : null,
			odds_one = data.length > 0 ? (this.supportCount(oddsInfo_one, oddsInfo_two, oddsInfo_thr, 0) * 100).toFixed(2) : '',
			odds_two = data.length > 0 ? (100 - odds_one).toFixed(2) : '';
		let columnData = [];
		if (oddsDetails_one.length != 0 && oddsDetails_two.length != 0 && oddsDetails_thr != 0) {
			let len;
			...
			for (var i = 0; i < len; i++) {
				...
				columnData[i].push(oddsDetails_thr[i]);
			}
		}
		if (flag == 1) {
			return <Fragment>
				...
				<div className={styles.content_top}>
					<div className={styles.amidithion}>{data.length > 0 ? oddsInfo_one.name : ''}</div>
					<div className={styles.amidithion}>平局</div>
					<div className={styles.amidithion}>{data.length > 0 ? oddsInfo_two.name : ''}</div>
				</div>
				<div className={styles.content_bottom}>
					<div className={styles.odds_rd}>
					...
					<div className={styles.odds_rd}>
						{data.length > 0 ? <AnimateNumber style={{ display: 'inline-block', lineHeight: '120px' }} value={oddsInfo_thr.odds == 1 ? `${oddsInfo_thr.odds}.01` : oddsInfo_thr.odds} className={styles.animate_num_3} /> : ''}
					</div>
					<div className={styles.odds_rd}>
